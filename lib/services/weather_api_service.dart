...
class WeatherApiService {
  ...
  Weather _mapOpenMeteoToWeather(Map<String, dynamic> data) {
    ...
    return Weather(
      ...
      feelsLikeC: (current['temperature_2m'] as num).round(),
      condition: condition,
  ...
  Future<Weather> fetchWeather(String query) async {
    // Legacy support or throw
    // Since we switched to coords, we can try to parse coords from query if possible, or just fail.
    // But WeatherDetailsScreen was updated to use getWeatherByCoordinates.
    // If anything else uses fetchWeather, it might break.
    // Let's implement a basic fallback or throw.
